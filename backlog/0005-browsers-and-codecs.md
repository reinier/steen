...
Native (non-Flatpak) `firefox` + `chromium` so they integrate with 1Password's
native-messaging and keep the `chrome-*` app_ids niri window-rules and dank-lader
rely on. Fedora's chromium links the system ffmpeg (`ffmpeg-free`, H.264/AAC
stripped), so add **`libavcodec-freeworld`** from RPM Fusion **free** (additive,
no base-package swap) or Teams WebRTC video and `<video>` mp4 break.
...
- `chromium` plays an H.264 `<video>` and Teams shows video; 1Password native
  messaging works in both browsers with no wrappers.
