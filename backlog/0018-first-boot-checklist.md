...
| 2026-07-20 | B: niri using DMS config | ❌ install had niri's **stock default** config.kdl (spawns waybar, stock binds) — DMS config never applied | Fixed (dotfiles) — `run_once_after_niri-dms-config.sh` backs up niri's default before `dms setup` (0014); `dms setup` is non-destructive so it wouldn't replace it otherwise |
